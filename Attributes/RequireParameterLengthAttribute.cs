...
public sealed class RequireParameterLength : ParameterPreconditionAttribute
{
    private readonly int _min = 0;
    private readonly int _max;
    public RequireParameterLength(int max = int.MaxValue)
        => _max = max;
    public RequireParameterLength(int min = 0, int max = int.MaxValue) : this(max)
        => _min = min;
